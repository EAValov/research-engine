...
public static class LanguageRegionSelectionPromptFactory
{
    public static Prompt Build(string query, IReadOnlyList<Clarification> clarifications)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("You MUST return ONLY a JSON object with exactly this shape:");
        sb.AppendLine("{");
        sb.AppendLine("  \"language\": \"xx\",");
        sb.AppendLine("  \"region\": \"Some Location String or null\"");
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine("Where:");
        ...
        sb.AppendLine("- \"region\" is a HUMAN-READABLE LOCATION STRING that will be passed directly as a `location` parameter to a search service, e.g.:");
        ...
        sb.AppendLine("- If the topic is strongly tied to a particular country or place, choose a concrete location string for that region.");
        ...
        sb.AppendLine("- If the topic is global and there is no clear regional focus, use:");
        sb.AppendLine("    { \"language\": \"en\", \"region\": null }");
        sb.AppendLine("  or, if you need a default country context for English, use:");
        sb.AppendLine("    { \"language\": \"en\", \"region\": \"United States\" }");
        sb.AppendLine();
        sb.AppendLine("IMPORTANT:");
        sb.AppendLine("- Do NOT return language names like \"German\" or \"English\" – only 2-letter codes in the \"language\" field.");
        sb.AppendLine("- Do NOT return formats like \"de-DE\" or \"en-GB\" in the \"region\" field – it must be a human-readable location string (e.g., \"Germany\", \"United Kingdom\").");
        sb.AppendLine("- Do NOT include any extra fields, comments, or text outside the JSON object.");
        sb.AppendLine("- If no specific region makes sense, set \"region\" to null.");
        sb.AppendLine();
        ...
        if (clarifications.Count > 0)
        {
            ...
            {
                sb.AppendLine($"- {c.Answer}");
            }
            sb.AppendLine();
        }

        var system = "You are a router that selects the best target language and location string for web research. " +
                    "Always respond with a single JSON object as instructed.";
